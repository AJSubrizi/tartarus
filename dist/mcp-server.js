...
import { cleanupTag, connectHarness, createWorktree, disconnectHarness, fanout, getJobLog, inspectJobs, inspectPath, killByTag, killJob, listAdapters, listJobsFiltered, listWorktrees, refreshHarnesses, removeWorktree, runOnHarness, snapshot, waitForJob, } from "./runtime.js";
...
export function createMcpServer() {
    ...
    server.tool("tartarus_help", "Read first. YOU orchestrate; Tartarus only spawns/inspects.", {}, async () => text(`TARTARUS — harness for the harnesses
...
Tartarus never auto-picks a winner.`));
    server.tool("tartarus_status", "Snapshot: harnesses, jobs, worktrees, adapters.", {}, async () => text(snapshot()));
    ...
    server.tool("tartarus_worktree_create", "Create isolated git worktree + env copy.", {
        branch: z.string(),
    ...
    server.tool("tartarus_preview_run", "Dry-run exact spawn argv.", {
        harnessId: z.string(),
        prompt: z.string(),
    ...
    server.tool("tartarus_run", "Spawn ONE harness unattended. YOU orchestrate next steps.", {
        harnessId: z.string(),
        prompt: z.string(),
        cwd: z.string().optional(),
    ...
    server.tool("tartarus_fanout", "Spawn same prompt on many harnesses. No auto-winner.", {
        prompt: z.string(),
        ...
        try {
            ...
            return text({
                ...result,
                message: "Fan-out started. Use tartarus_wait / tartarus_inspect_jobs. YOU pick the winner.",
            });
        }
        catch (e) {
            return text({ error: e instanceof Error ? e.message : String(e) });
    ...
    server.tool("tartarus_job", "Job status + in-memory log tail + commandLine.", { jobId: z.string() }, async ({ jobId }) => {
    ...
    server.tool("tartarus_wait", "Block until job finishes (or timeout). Convenience for the orchestrator.", {
        jobId: z.string(),
        timeoutMs: z.number().optional(),
    ...
    server.tool("tartarus_inspect", "Git facts for a path/worktree: status, diffstat, patch preview. No ranking.", {
        path: z.string(),
        patchLimit: z.number().optional(),
    }, async ({ path, patchLimit }) => text(inspectPath(path, { patchLimit })));
    server.tool("tartarus_inspect_jobs", "Git facts for jobs (by tag or ids) so YOU can compare and pick a winner.", {
        tag: z.string().optional(),
    ...
    server.tool("tartarus_cleanup_tag", "Kill running jobs with tag + remove their worktrees. YOU call this on losers.", {
        tag: z.string(),
