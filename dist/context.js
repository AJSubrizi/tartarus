...
function readSafe(path, max = 12_000) {
    try {
 ...
 */
export function materializeContextFiles(root, worktree, files) {
    const copied = [];
...
export function buildHandoffSummary(job) {
    ...
    if (filesChanged != null) {
        lines.push(`- **Diff:** ${filesChanged} files, +${additions ?? 0}/−${deletions ?? 0}`);
    }
    lines.push("", "### Log excerpt", "```", logExcerpt || "(empty)", "```");
    lines.push("", "### Instructions for next agent", "Continue from this state. Do not redo completed work. Fix only remaining gaps.");
    return {
        jobId: job.id,
...
export function renderBrief(opts) {
    ...
    if (guides.length) {
        parts.push(``, `## Project guides (read if relevant)`, ``, `These files exist in the workspace:`);
    ...
    parts.push(``, `## Done criteria`, ``, `- Implement the goal without violating constraints`, `- Keep changes focused`, `- Prefer running available tests/linters when relevant`, ``);
    return {
        brief: parts.join("\n"),
        goal,
...
export function writeBriefFile(brief, cwd, jobId) {
 ...
 */
export function promptForAdapter(briefPath, goal) {
    return [
...
export function mergeContext(prompt, context) {
