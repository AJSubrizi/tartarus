...
export function mcpConfigObject(
  ...
  mcpServers: {
    tartarus: { command: string; args: string[] };
  };
} {
  const launch =
    mode === "local" ? mcpLaunchLocal(repoRoot) ?? mcpLaunchNpx() : mcpLaunchNpx();
  return {
    mcpServers: {
      tartarus: {
...
export function codexAddCommand(mode: "npx" | "local" = "npx"): string {
...
export function printSetupGuide(): string {
