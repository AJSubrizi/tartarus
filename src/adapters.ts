...
export function maybePromptFile(prompt: string, prefix: string): {
  prompt: string;
  file?: string;
  useFile: boolean;
} {
  ...
  const file = join(
    dir,
    `${prefix}_${Date.now()}_${randomBytes(3).toString("hex")}.md`,
  );
...
const claude: Adapter = {
  ...
  versionArgs: ["--version"],
  notes:
    "Uses -p/--print + --dangerously-skip-permissions for unattended worktree runs.",
  build(input) {
...
const codex: Adapter = {
  ...
  versionArgs: ["--version"],
  notes:
    "Uses `codex exec` non-interactive. Safer mode: sandbox workspace-write. Default unattended: bypass approvals+sandbox (for isolated worktrees).",
  build(input) {
...
const grok: Adapter = {
  ...
  versionArgs: ["--version"],
  notes:
    "Uses -p/--single headless + --permission-mode bypassPermissions (or auto if safer).",
  build(input) {
...
const pi: Adapter = {
  ...
  versionArgs: ["--version"],
  notes:
    "pi.dev coding agent. Headless: -p/--print --mode text --approve (trust project files for the run).",
  build(input) {
...
const zero: Adapter = {
  ...
  versionArgs: ["version"],
  notes:
    "Gitlawb/zero coding agent (`zero exec`). If PATH has AgentMesh zero instead, falls back to `zero run`.",
  build(input) {
...
function detectZeroVariant(
  command: string,
): "gitlawb" | "agentmesh" | "unknown" {
  const help = spawnSync(command, ["--help"], {
    encoding: "utf8",
    timeout: 5_000,
  });
  const out = `${help.stdout ?? ""}${help.stderr ?? ""}`.toLowerCase();
  if (out.includes("agentmesh")) return "agentmesh";
  ...
  if (out.includes("coding agent") || out.includes("gitlawb")) return "gitlawb";
  return "unknown";
}
...
export function buildHarnessRun(
  kind: HarnessId,
  input: RunBuildInput,
): BuiltRun {
...
export function adapterCatalog(): Array<{
  kind: HarnessId;
  label: string;
  binaries: string[];
  notes: string;
  ...
  return (Object.keys(ADAPTERS) as HarnessId[])
    ...
    .map((kind) => {
      ...
      const version = path
        ? probeVersion(
            "command" in resolved ? resolved.command : path,
            a.versionArgs,
          )
        : undefined;
      return {
