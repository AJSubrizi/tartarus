...
function flushLogTail(jobId: string, force = false): void {
  const run = active.get(jobId);
  if (!run || (!run.logBuf && !force)) return;
...
function pushLog(jobId: string, chunk: string): void {
...
function notifyWaiters(jobId: string): void {
  ...
  for (const w of list) {
    ...
    if (job) w.resolve(job);
    else w.reject(new Error("job missing"));
  }
}
...
export function disconnectHarness(id: string): boolean {
...
function releaseHarness(harnessId: string): void {
  const busy = store
    .listJobs(80)
...
export function killByTag(tag: string): { killed: string[] } {
  const killed: string[] = [];
  for (const j of store.listJobs(150)) {
    if (j.tag === tag && !terminal(j.status)) {
      if (killJob(j.id)) killed.push(j.id);
    }
  }
  return { killed };
}

export function waitForJob(jobId: string, timeoutMs = 60 * 60_000): Promise<Job> {
  const job = store.getJob(jobId);
  if (job && terminal(job.status)) return Promise.resolve(job);
...
export function inspectJobs(opts: {
  ...
  jobs: Array<{ jobId: string; harnessId: string; status: string; path?: string }>;
} {
  let jobs = store.listJobs(100);
  if (opts.tag) jobs = jobs.filter((j) => j.tag === opts.tag);
...
export function cleanupTag(
  tag: string,
  opts?: { killRunning?: boolean; removeWorktrees?: boolean },
): {
  killed: string[];
  ...
  if (opts?.removeWorktrees !== false) {
    for (const j of store.listJobs(150).filter((x) => x.tag === tag)) {
      ...
      if (r.ok) removedWorktrees.push(j.worktree);
      else errors.push(`${j.worktree}: ${r.error}`);
    }
  }
...
export function getJobLog(jobId: string, tail = 20_000) {
  const job = store.getJob(jobId);
...
for (const h of store.listHarnesses()) {
  if (h.status === "busy") store.upsertHarness({ ...h, status: "ready" });
}
