...
export function inspectPath(
  ...
  for (const block of [numstat, numstatStaged]) {
    for (const line of block.split("\n")) {
      ...
      if (!Number.isNaN(ai)) additions += ai;
      if (!Number.isNaN(di)) deletions += di;
      files += 1;
    }
  }
  ...
  return {
    ...
    diffStat,
    patchPreview:
      combinedPatch.length > patchLimit
        ? combinedPatch.slice(0, patchLimit) + "\n…[truncated]"
        : combinedPatch,
  };
}

function empty(path: string, error: string): InspectResult {
  return {
    path,
    isGit: false,
    dirty: false,
...
export function inspectMany(paths: string[]): {
